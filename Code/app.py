...
def db_conn():
    try:
        conn = psycopg2.connect(database="toiletDB", host="localhost", user="postgres", password="292001",
                                port=5432)
        return conn
...
def get_handicapped():
    ...
    if con:
        try:
            cur = con.cursor()
            cur.execute(
                "select vh.location, vh.hc_true as no_of_toilet, tl.longitude, tl.latitude from vhandicapped as vh left join tbllocation as tl on vh.location=tl.location order by hc_true desc")
            data = cur.fetchall()
...
def data_handicapped():
    ...
    return render_template('handicapped_page.html', labels=location_list, ishandicapped=ishandicapped_list, div_map=div)
...
def get_freetoilet():
    ...
    if con:
        try:
            cur = con.cursor()
            cur.execute(
                "SELECT vf.location, vf.free_toilet, tl.longitude, tl.latitude FROM public.vfreetoilet as vf left join tbllocation as tl on vf.location=tl.location order by free_toilet desc, location asc")
            data = cur.fetchall()
            column_names = [desc[0] for desc in cur.description]
            cur.close()
            con.close()
...
def get_open24hours():
    ...
    if con:
        try:
            cur = con.cursor()
            cur.execute(
                "SELECT tl.location, v.open24hours, tl.longitude, tl.latitude FROM public.visopen24hours as v left join tbllocation as tl on v.location=tl.location order by open24hours desc")
            data = cur.fetchall()
            column_names = [desc[0] for desc in cur.description]
            cur.close()
            con.close()
